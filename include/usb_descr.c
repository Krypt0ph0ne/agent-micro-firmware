...
__code USB_DEV_DESCR DevDescr = {
  ...
  .iSerialNumber      = 0,                      // no static per-device serial number
...
__code USB_CFG_DESCR_HID CfgDescr = {
  ...
  .ep1OUT = {
   ...
   .RawInterface = {
    ...
    .bAlternateSetting  = 0,                      // value used to select alternative setting
    .bNumEndpoints      = 2,                      // interrupt IN + OUT
    .bInterfaceClass    = USB_DEV_CLASS_HID,      // interface class: HID (0x03)
  ...
  // Endpoint Descriptor: Endpoint 2 (IN, Interrupt). It normally NAKs; its
  // presence makes the vendor HID interface standards-compliant on macOS.
  .ep2IN = {
...
__code uint8_t RawHIDReportDescriptor[] = {
    ...
    0x09, 0x61,       // Usage (Agent Micro control)
    0xA1, 0x01,       // Collection (Application)
    0x09, 0x62,       // Usage (Command output)
    0x15, 0x00,       // Logical Minimum (0)
    0x26, 0xFF, 0x00, // Logical Maximum (255)
    0x95, 32,         // Report Count (32 bytes)
    0x75, 0x08,       // Report Size (8 bits)
    0x91, 0x02,       // Output (Data, Variable, Absolute)
    0x09, 0x63,       // Usage (Status and physical input events)
    0x95, 32,         // Report Count (32 bytes)
    0x75, 0x08,       // Report Size (8 bits)
