        ...
        case USB_GET_DESCRIPTOR:
          switch(USB_setupBuf->wValueH) {
            ...
            case USB_DESCR_TYP_REPORT:
              // Report descriptor index is normally zero for every HID
              // interface. The interface number lives in wIndex, so using
              // wValueL here caused interface 1 to receive the keyboard
              // descriptor as well.
              if(USB_setupBuf->wIndexL == 0) {
                pDescr = USB_REPORT_DESCR;
                len = CfgDescr.hid0.wDescriptorLength;
              } else if(USB_setupBuf->wIndexL == 1) {
