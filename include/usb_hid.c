...
volatile __bit HID_EP2_writeBusyFlag = 0; // raw HID upload pointer busy flag
...
// Send a report to the host through the vendor Raw HID interface.
void HID_sendRawReport(__xdata uint8_t *buf, uint8_t len) {
  uint8_t i;
  while (HID_EP2_writeBusyFlag)
    ;
  for (i = 0; i < len; i++)
    EP2_SEND_buffer[i] = buf[i];
  UEP2_T_LEN = len;
...
void HID_setup(void) {
  ...
  UEP2_CTRL = bUEP_AUTO_TOG    // EP2 Auto flip sync flag
              | UEP_T_RES_NAK  // passive EP2 IN normally has no report
  ...
  UEP2_3_MOD = bUEP2_RX_EN | bUEP2_TX_EN; // EP2 OUT + passive IN
}
...
uint8_t HID_statusLed(void) { return statusLed; }
