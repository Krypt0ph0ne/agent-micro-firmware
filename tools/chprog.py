...
def _main():
    if len(sys.argv) == 2 and sys.argv[1] == '--detect':
        try:
            ...
            sys.exit(1)
        print('READ-ONLY CHECK COMPLETE. Nothing erased or written.')
        sys.exit(0)

    if len(sys.argv) != 3 or sys.argv[1] != '--flash':
        sys.stderr.write('Usage: chprog.py --detect | --flash firmware.bin\n')
        sys.exit(1)
    ...
    try:
        ...
        print('Erasing chip ...')
        isp.erase()
        print('Flashing', sys.argv[2], 'to', isp.chipname, '...')
